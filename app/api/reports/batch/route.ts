...
export async function POST(req: Request) {
  ...
  const tasks = rows.map((row, i) =>
    limit(async () => {
      ...
      const originLabel = `${originCity}${originCounty ? ", " + originCounty : ""} | KGM: ${KGM_URL}`;
      const destLabel = `${destinationCity}${destinationCounty ? ", " + destinationCounty : ""} | KGM: ${KGM_URL}`;
      ...
      try {
        ...
        // Call Gemini AI for route analysis
        ...
        await supabase.from("reports").update({ analysis, status: "ready", error_message: null }).eq("id", reportInsert.id);

        // Update batch item
        if (batchItem?.id) {
          await supabase.from("batch_items").update({ status: "ready", report_id: reportInsert.id }).eq("id", batchItem.id);
        }

        return { row: i, status: "ready" };
      } catch (err: any) {
        ...
        return { row: i, status: "failed", error: err?.message };
      }
    })
  );
